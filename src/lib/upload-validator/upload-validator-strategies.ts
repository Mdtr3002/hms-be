...
export class BookfairUploadValidation implements UploadValidatorStrategy {
    validate(files: Express.Multer.File[]): void {
        if (files.length > 1) {
            throw new Error(
                `You can only upload at most one file for this operation`
            );
        }
        if (!files[0].mimetype.startsWith("image/")) {
            throw new Error(`Only image files are allowed for this operation`);
        }
    }
}
